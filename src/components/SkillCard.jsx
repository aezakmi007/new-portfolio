...
const SkillCard = ({ skill, percentage, color, move }) => {
  return (
    <div
      className={`border border-[rgba(255, 255, 255, 0.3)] rounded-md px-8 py-3 cursor-pointer groupd w-full flex items-center justify-between relative gap-2`}
      style={{
    ...
    >
      <div className="flex-1 flex-col items-start justify-start flex gap-2">
        ...
        <div className="w-full h-1 rounded-md overflow-hidden bg-[#ffffff33] relative">
          <motion.div
      ...
      >
       ...
       className="w-full absolute bottom-0 left-0 flex items-center justify-center">
        ...
        <p
        className="text-bgPrimary z-10 font-sans font-bold tracking-wider"
        >{percentage}</p>
        </motion.div> 
      </div>
    </div>
